...
public class ExamBuilderService {
     ...
     * @param wb contains sheet Part1 ~ Part7.<br/>
     * No space is sheet name.
     * @return
     * @throws IOException
     * @throws FormatException 
     */
    ...
    private void processSheet(Sheet sheet, int colIdx) throws IOException, FormatException {
        ...
        for (int rowIdx = 1; rowIdx <= lastRowNum; rowIdx++) {
            ...
            // Debug
            if (sheetName.contentEquals("Part7")) {
                log.debug("rowIdx=" + rowIdx + ";lastRowNum=" + lastRowNum);
            }
            if (sheetName.contentEquals("Part7") && rowIdx >= 41) {
                log.debug("");
            }
...
//                    log.error(String.format("Objective at sheet '%s', row '%d', col '%d' is not valid format: '%s'.", sheet.getSheetName(), rowIdx + 1, colIdx + 1, objective));
            ...
            if (questionText == null) {
                log.warn(String.format("Content not foud in sheet '%s', row '%d', col '%d'.", sheetName, rowIdx + 1, colIdx + 1));
                break;
            }

            // Debug
            if (sheetName.contentEquals("Part7") && rowIdx == 46) {
                log.debug("");
            }

            if (questionText.trim().startsWith("{")) { // Start of json data
                // Check local media
                ...
                if ((CommonUtil.isNNandNB(imageLocalPath)) && (!new File(imageLocalPath).exists())) {
                    throw new IOException(String.format("Media in sheet '%s', row '%d', col '%d' '%s' is not existed.", sheetName, rowIdx + 1, colIdx + 1, imageLocalPath));
                }
                
                if ((CommonUtil.isNNandNB(audioLocalPath)) && (!new File(audioLocalPath).exists())) {
                    throw new IOException(String.format("Media in sheet '%s', row '%d', col '%d' '%s' is not existed.", sheetName, rowIdx + 1, colIdx + 1, audioLocalPath));
                }
            }
